    ...
    def data_association(self, dets):
        ...
        if num_det*num_trk > 0:
            ...
            for j in range(num_trk):
                trk_idx = trackidx[j]
                cost_matrix[:,j], relative_ious[:,j], relative_ps[:,j] = self.trackers[trk_idx].distance(det_ys, det_covs)
            ...
            for i,j in matched_indices:
                ...
                trk_idx = trackidx[j]
                self.trackers[trk_idx].update(dets[det_idx].y, dets[det_idx].R, relative_ious[i,j], relative_ps[i,j])
