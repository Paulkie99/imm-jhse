...
def getUVError(box):
    u = 1360 / box[2]#0.3*box[3]
    v = 764 / box[3]#0.3*box[3]
    u, v = max(u, 0.1 * box[2]), min(v, 0.1 * box[3])
    u, v = 0.2 * box[2], 0.2 * box[3]
    # if u>13:
    #     u = 13
    # elif u<2:
    #     u = 2
...
class Mapper(object):
    ...
    def update(self, tracks, dets):
        ...
        jacobian = np.zeros((feet_measurements.shape[0] * meas_size, feet_measurements.shape[0] * state_size + 8))
        temp_cov = np.zeros((feet_measurements.shape[0] * state_size + 8, feet_measurements.shape[0] * state_size + 8))
        uv_projs = []
        track_means = []
        for det_idx, track in enumerate(tracks):
            xy1 = np.zeros((3, 1))
            xy = track.kf.x
            xy1[:2, :] = xy[[0, 2]]
            xy1[2, :] = 1
            ...
            jacobian[det_idx * meas_size: (det_idx + 1) * meas_size,
                     det_idx * state_size: (det_idx + 1) * state_size] = dU_dX
            jacobian[det_idx * meas_size: (det_idx + 1) * meas_size, -8:] = dU_dA

            temp_cov[det_idx * state_size: (det_idx + 1) * state_size, 
                     det_idx * state_size: (det_idx + 1) * state_size] = track.kf.P[:state_size, :state_size]
            
            temp_cov[det_idx * state_size: (det_idx + 1) * state_size, -8:] = track.kf.P[:state_size, -8:]
            temp_cov[-8:, det_idx * state_size: (det_idx + 1) * state_size] = track.kf.P[-8:, :state_size]
        ...
        kalman_gain = temp_cov @ jacobian.T @ np.linalg.inv(projected_cov)

        old_means = np.r_[np.array(track_means).reshape((state_size * feet_measurements.shape[0])), self.A.T.flatten()[:-1]] 
        new_mean = old_means + (kalman_gain @ innov.reshape((kalman_gain.shape[1], )))
        ...
        for det_idx, track in enumerate(tracks):
            track.kf.x[:4, 0] = new_mean[det_idx * state_size: (det_idx + 1) * state_size]
            track.kf.x[-8:, 0] = new_mean[-8:]
            track.kf.P[:state_size, :state_size] = covariance[det_idx * state_size: (det_idx + 1) * state_size, 
                                                               det_idx * state_size: (det_idx + 1) * state_size]
            track.kf.P[:state_size, -8:] = covariance[det_idx * state_size: (det_idx + 1) * state_size, -8:]
            track.kf.P[-8:, :state_size] = covariance[-8:, det_idx * state_size: (det_idx + 1) * state_size]
